...
/* !!!!!!!! REVIEW !!!!!!!! */
  ...
  SPI.begin();	//Particle uses SS by default
  initSS();		//Particle sets SS high by default

  #if defined(PLATFORM_ID) && ((PLATFORM_ID == 0) || (PLATFORM_ID == 6))
  // Start Particle SPI
  SPI.setBitOrder(MSBFIRST);
  SPI.setClockDivider(SPI_CLOCK_DIV16); //4.5Mhz opposed to Arduino's 4Mhz
  SPI.setDataMode(SPI_MODE0);
  #else
  // Start Arduino SPI
  SPI.setBitOrder(MSBFIRST);
  SPI.setClockDivider(SPI_CLOCK_DIV4); //4.5Mhz opposed to Arduino's 4Mhz
  SPI.setDataMode(SPI_MODE0);
  #endif	  
#else
  SPI.begin(SPI_CS);
